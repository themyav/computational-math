 ...
 */
void gauss(int n, vector<vector<ld>>&matrix, vector<ld>&b){

    print_message("Треугольная матрица");
    ...
    print_message("Определитель матрицы");
    cout << count_determinant(n, matrix) << endl;

    int solution = check_rang(n, matrix, b);
    if(solution == NO_SOL) {
        print_message("Система не имеет решений");
        exit(0);
    }
    else if(solution == ENDLESS_SOL){
        print_message("Система имеет бесконечное множество решений");
        exit(0);
    }

    print_message("\nВектор значений x_i");
    vector<ld>x = count_x(n, matrix, b);
    print_vector(n, x, "x");

    print_message("Вектор невязок");
    vector<ld>discrepancy = count_discrepancy(n, matrix, b, x);
    print_vector(n, discrepancy, "r");
}
